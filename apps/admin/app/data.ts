...
////////////////////////////////////////////////////////////////////////////////
// This can be used as an alternative to the in-memory implementation above
// by uncommenting the code below and importing these functions instead

import { createDb, contacts } from "@no-notes/db";
...
export async function getDrizzleContacts(query?: string | null) {
  ...
  if (query) {
    contactList = await db.select().from(contacts).where(
...
export async function updateDrizzleContact(id: string, updates: ContactMutation) {
  ...
  if (updates.first !== undefined || updates.last !== undefined) {
    const firstName = updates.first !== undefined ? updates.first : contact.first;
...
async function initializeDrizzleData() {
  const existingContacts = await db.select({ count: contacts.id }).from(contacts);
